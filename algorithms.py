...
def gcd(a, b, verbose : bool = False):
    r"""
        Compute the GCD using the Euclidean algorithm described in theorem 1.7
    """
    if verbose:
...
def bezout(a, b, verbose : bool = False, lenstra=False):
    r"""
        Compute the Bezout coefficients using the extended Euclidean algorithm described in theorem 1.11
...
def fast_pow(g, A, N, verbose : bool = False):
    r"""
        Exponentiating by squaring aka square-and-multiply aka binary exponentiation. Computes
            g^A (mod N)
...
def mr_test(n, a, verbose : bool = False):
    r"""
        Miller-Rabin test for composite numbers described in table 3.2. Failure indicates that a is not a Miller-Rabin
...
def pollard_pminus(N, k=100, verbose : bool = False):
    r"""
        Pollard"s p-1 factorization algorithm described in table 3.3. Finds a nontrivial factor of N by searching
        up to j=k
    """
    a = 2
    tex = r"\begin{align*}" + "\n"
...
def shanks_bsgs(g, h, p, verbose : bool = False):
    r"""
        Shanks"s Babystep-Giantstep algorithm to solve the discrete logarithm problem
            g^x = h (mod p)
...
def pt_add(P, Q, p, verbose : bool = False, lenstra=False):
    r"""
...
def pt_double(P, p, a, verbose : bool = False, lenstra=False):
    r"""
...
def double_add(P, p, a, b, n, verbose : bool = False, lenstra=False):
    r"""
...
def lenstras_determ(N, A, a, b, k=100, verbose : bool = False):
    r"""
        Lenstra"s elliptic curve factorization algorithm as described in table 6.8:
            Factor integer N using point P = (a, b)
...
def lenstras(N, k=100, verbose : bool = False):
    r"""
        Randomized wrapper for Lenstra"s algorithm. Search up to j=k
    """
    for j in range(k):
